...
class BookingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _occupiedSlots = MutableLiveData<List<Booking>>()
    val occupiedSlots: LiveData<List<Booking>> = _occupiedSlots
        ...
        viewModelScope.launch {
            try {
                ...
                result.onSuccess { bookings ->
                    val bookingList = bookings.mapNotNull { mapToBooking(it) }
                    Log.d("BookingViewModel", "Found ${bookingList.size} occupied slots")
                    _occupiedSlots.value = bookingList
                }.onFailure { e ->
        ...
        viewModelScope.launch {
            try {
                ...
                firebaseBookingRepo.getUserBookings(userId).collect { firebaseBookings ->
                    // Convert to Booking
                    val bookingList = firebaseBookings.mapNotNull { mapToBooking(it) }

                    Log.d("BookingViewModel", "Loaded ${bookingList.size} bookings")
                    _userBookings.postValue(bookingList)
                }
            } catch (e: Exception) {
    ...
    /**
     * Convert Firebase Map data to Booking
     */
    private fun mapToBooking(firebaseData: Map<String, Any>): Booking? {
        return try {
